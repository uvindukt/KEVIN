...
<title>KEVIN FASHIONS | Disputes</title>
...
<%@ page import="javax.servlet.http.HttpSession, kevin.model.User, java.util.ArrayList, kevin.model.Item, kevin.service.ItemService" %>
<% 
...
<body>
    ...
    <div id="overlay"></div>
        ...
        <nav class="navbar">
            ...
            <%
				if (logged.getAttribute("user") != null) {
					out.print("<a id='acb' class='nvbut' href='dispute.jsp'><i class='fa fa-exclamation-triangle'></i>&ensp;Disputes</a>");
    ...
    <div class="track-container">
    	<div class="track-cont1">
    		<h2>Add Complaint</h2>
    		<form method="post" action="DisputeServlet" enctype="multipart/form-data">
    			<textarea required autocomplete="off" class="textbox" type="text" placeholder="Description" name="description" style="width: 40vw; height: 6vw"></textarea>
    			<br><br><div class="upload-btn-wrapper">
    				<input required class="textbox" onchange="alertFilename()" type="file" id="inputfile" accept="image/*" name="image">
  					<button id="up" class="btn">Upload a file</button>
    			</div><br><br>
    			<button class="button" type="submit" value="Submit"><i class="fas fa-save"></i>&ensp;Save</button>&emsp;<button class="button" type="reset" value="Clear"><i class="fas fa-times"></i>&ensp;Clear</button>
    		</form>
    	</div>
    	<div class="track-cont2" style="padding-top: 3vh">
    		<h2>Complaints</h2>
    		<form method="post" action="GetDisputeServlet" id="ofl"><input type="text" hidden required name="id" id="osub" value=""><select onchange="getDisputeDetails(document.getElementById('ofl'));" required class="select" style="width: 20vw; margin-bottom: 2vw" id="olist"><option value="" disabled selected>Select Compliant</option></select></form>
    		<form method="post" action="DeleteDisputeServlet" id="dor"><input type="text" hidden required name="id" id="orid" value=""><button type="button" onclick="Dispute.render('Enter New Description', 'EditDisputeServlet')" class="button"><i class="fa fa-edit"></i>&ensp;Edit</button>&emsp;<button type="button" class="button" onclick="Confirm.render('Are you sure want to delete this complaint?', 'dor')"><i class="fa fa-trash"></i>&ensp;Delete</button></form>
    	</div>
